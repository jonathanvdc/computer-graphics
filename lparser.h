...
namespace LParser
{
	...
	*/
	...
	{
		ReplacementRule() : candidates(), totalWeight() { }
		ReplacementRule(std::vector<ReplacementCandidate> Candidates) : candidates(Candidates), totalWeight() { }

		/**
		 * \brief Gets the sum of the replacement candidates' weights.
		 */
		double GetTotalWeight() const;

		/**
		 * \brief Applies the replacement rule for a random value 0.0 <= Value <= 1.0.
		 */
		std::string const& Replace(double Random) const;

		std::vector<ReplacementCandidate> GetCandidates() const { return this->candidates; }
	...
	class LSystem
	{
		...
		public:
			...
			/**
			* \brief Returns the replacement rule for a given character of the Alphabet
			*
			* \param c 	the character of the alphabet
			*
			* \return	replacement rule
			*/
			const ReplacementRule& get_replacement_rule(char c) const;
		...
		protected:
			/**
			 * Generates a random value within [0.0, 1.0].
			 */
