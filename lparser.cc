 ...
 *
 * --------------------------------------------------------------------
 * Stochastic replacement rules by Jonathan Van der Cruysse
 */
...
namespace
{
	...
	LParser::ReplacementRule parse_rule(char alphabet_char, std::set<char> const& alphabet, stream_parser& parser, bool parse2D)
	{
		std::vector<LParser::ReplacementCandidate> candidates;
		char peek = ';';
		while (peek == ';')
		{
			...
			if (parser.peekChar() == '-')
			{
				parser.assertChars("-");
				parser.skip_comments_and_whitespace();
				double weight = parser.readDouble();
				parser.skip_comments_and_whitespace();
				candidates.push_back({ rule, weight });
			}
			else
			{
				candidates.push_back({ rule, 1.0 });
			}
			peek = parser.peekChar();
			if (peek == ';')
			{
				parser.assertChars(";");
				parser.skip_comments_and_whitespace();
	...
	void parse_rules(std::set<char> const& alphabet, std::map<char, LParser::ReplacementRule>& rules, stream_parser& parser, bool parse2D)
	{
		parser.skip_comments_and_whitespace();
		...
		while (true)
		{
			...
			parser.assertChars("->");
			parser.skip_comments_and_whitespace();
			LParser::ReplacementRule rule = parse_rule(alphabet_char, alphabet, parser, parse2D);
...
double LParser::ReplacementRule::GetTotalWeight() const
{
	if (totalWeight == 0.0)
	{
		double sum = 0.0;
...
std::string const& LParser::ReplacementRule::Replace(double Random) const
{
	double modifiedRand = Random * this->GetTotalWeight();
	double sum = 0.0;
	for (auto& candidate : this->candidates)
	{
		sum += candidate.Weight;
	...
	return this->candidates.at(this->candidates.size() - 1).Value;
}
...
double LParser::LSystem::generate_random() const
{
...
{
	assert(get_alphabet().find(c) != get_alphabet().end());
	auto& rule = replacementrules.find(c)->second;
	double random = this->generate_random();
	return rule.Replace(random);
}
