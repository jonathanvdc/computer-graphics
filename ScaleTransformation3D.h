#pragma once
#include <memory>
#include "ITransformation.h"
#include "Vector3.h"

namespace Engine
{
    /// \brief A three-dimensional scaling transformation.
    struct ScaleTransformation3D : public virtual ITransformation<Vector3>
    {
        ScaleTransformation3D(Vector3 Scale);

        /// \brief Inverts this transformation.
        std::shared_ptr<ITransformation<Vector3>> Invert() const override;

        /// \brief Applies this transformation to the given value.
        Vector3 Transform(Vector3 Value) const override;

        /// \brief The scaling vector for this scaling transformation.
        Vector3 Scale;
    };
}